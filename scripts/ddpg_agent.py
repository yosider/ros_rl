...
class DDPG_agent():
    def __init__(self):
        self.sub_stepinfo = rospy.Subscriber("/ros_rl/stepinfo", Stepinfo, self.predict_action, queue_size=1, buff_size=2**24)
        ...
        self.actor = ActorNetwork(self.sess, STATE_SIZE, ACTION_SIZE, ACTION_BOUND, BATCH_SIZE, TAU, ACTOR_LEARNING_RATE)
        self.critic = CriticNetwork(self.sess, STATE_SIZE, ACTION_SIZE, BATCH_SIZE, TAU, CRITIC_LEARNING_RATE)
        self.replay_buffer = ReplayBuffer(BUFFER_SIZE)
        self.graph = tf.get_default_graph()

    def predict_action(self, msg):
        with self.graph.as_default():
            next_state, reward, done, info = unpack_stepinfo(msg)
    ...
    def train(self, msg):
        s_batch, a_batch, r_batch, t_batch, s2_batch = msg.data
        ...
        # train critic
        critic.train(s_batch, a_batch, target_R.reshape(BATCH_SIZE, 1))
